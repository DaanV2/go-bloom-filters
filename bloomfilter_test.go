...
// filterFactory is a function that creates a new bloom filter with the given options.
type filterFactory func(opts ...bloomfilters.BloomFilterOptions) (bloomfilters.IBloomFilter, error)

// testFilters returns a list of filter factories to test against.
func testFilters() map[string]filterFactory {
	return map[string]filterFactory{
		"BloomFilter": func(opts ...bloomfilters.BloomFilterOptions) (bloomfilters.IBloomFilter, error) {
			return bloomfilters.NewBloomFilter(opts...)
		},
		"ConcurrentBloomFilter": func(opts ...bloomfilters.BloomFilterOptions) (bloomfilters.IBloomFilter, error) {
			return bloomfilters.NewConcurrentBloomFilter(opts...)
		},
	}
}
...
func Test_NewBloomFilter_Valid(t *testing.T) {
	for name, factory := range testFilters() {
...
// Fuzz test for ConcurrentBloomFilter Add and Test
