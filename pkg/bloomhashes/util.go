...
func PutUint64(b []byte, values []uint64) int {
	...
	tmp := unsafe.Slice((*uint64)(unsafe.Pointer(&b[0])), l) // nolint:gosec // #nosec G103 -- This is safe because we are only reading from the byte slice to extract uint64 values.
	copy(values[:l], tmp)
